...
test('compiles the counter: hoisted template, walks, bindings — no html`` left', () => {
  ...
  expect(out).toContain('from "@amojs.dev/core/compiled"');
  ...
  expect(out.indexOf('@amojs.dev/core/compiled')).toBeGreaterThan(
    out.indexOf("'@amojs.dev/core/runtime'"),
  );
  expect(out.indexOf("'@amojs.dev/core/runtime'")).toBeGreaterThan(-1);
});
