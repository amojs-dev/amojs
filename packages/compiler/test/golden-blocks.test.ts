...
const LIST = [
  "import { signal, html, each } from '@amojs.dev/core';",
  "export { flushSync } from '@amojs.dev/core';",
  'export function App() {',
...
test('GOLDEN keyed list: raw and compiled behave identically', async () => {
  ...
  expect(compiled).not.toContain('html`');
  expect(compiled).toContain("import { signal, each } from '@amojs.dev/core/runtime';"); // html stripped, each kept, parser-free entry
