...
const AMO_APP = [
  "import { signal, html, mount, each } from '@amojs/core';",
...
test('shipped bytes: the same app in AmoJS vs Lit', async () => {
  ...
  try {
    ...
    const compiled = compileModule(AMO_APP)
      .replaceAll("'@amojs/core/runtime'", `'${join(CORE_SRC, 'runtime.js')}'`)
      .replaceAll('"@amojs/core/compiled"', `"${join(CORE_SRC, 'compiled.js')}"`)
      .replaceAll("'@amojs/core'", `'${join(CORE_SRC, 'index.js')}'`);
